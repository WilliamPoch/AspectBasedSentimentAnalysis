...
class Asp_Sentiment_classifiers:
    ...
    def __init__(self):
        ...
        # Overall
        json_file = open("models/overall_sentiment_model.json", 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        overall_model = model_from_json(loaded_model_json)
        overall_model.load_weights("models/overall_sentiment_model.h5")

        # Aspect
        # lda_model = gensim.models.LdaModel.load('models/lda.model')
        json_file = open("models/aspect_model.json", 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        aspect_model = model_from_json(loaded_model_json)
        aspect_model.load_weights("models/aspect_model.h5")
    ...
    def aspect_classifier(self, reviews_df, threshold=0.06):
        """
